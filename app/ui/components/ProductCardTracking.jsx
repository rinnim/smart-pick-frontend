...
const ProductCardTracking = ({ product, trackingPrice, handleClose }) => {
  const onClose = async () => {
    try {
        ...
        <div className="flex w-full flex-col gap-1">
          ...
          <div className="mb-2 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 text-base md:text-xl">
              ...
              <p className="text-md font-bold">৳{trackingPrice}</p>
            </div>
            <div
              className={`rounded-full font-bold ${
                product?.price <= trackingPrice ? "bg-green-500" : "bg-red-500"
              } px-2 py-1 text-xs md:px-4 md:py-2 md:text-base`}
            >
              {product?.price <= trackingPrice
                ? "Target Reached"
                : "Target Not Reached"}
            </div>
          </div>
...
export default ProductCardTracking;
